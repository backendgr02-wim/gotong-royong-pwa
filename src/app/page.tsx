...
export default async function Beranda() {
  ...
  // Guest browsing — tanpa login, seperti TikTok
  if (!user) {
    return <GuestBeranda />;
  }
...
/** Tampilan untuk pengunjung yang belum login — berselancar tanpa akun. */
function GuestBeranda() {
  return (
    <div className="flex min-h-dvh flex-col bg-white">
      ...
      <div className="space-y-4 p-4">
        {/* Hero CTA */}
        <Card className="bg-gradient-to-br from-primary to-primary-deep text-white text-center">
          <Logo size={48} className="mx-auto text-white" />
          <h2 className="mt-3 text-xl font-bold">Komunitasmu dalam Satu Aplikasi</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { icon: TrendingUp, label: "Transparansi Kas", desc: "Catat & pantau keuangan" },
            { icon: Clock, label: "Jadwal Sholat", desc: "Metode Kemenag RI" },
            { icon: CalendarDays, label: "Kegiatan", desc: "RSVP & pengingat" },
            { icon: HeartHandshake, label: "Donasi & Iuran", desc: "Transfer + foto bukti" },
          ].map((f) => {
            const Icon = f.icon;
            return (
        ...
        {/* CTA Masuk */}
        <Card className="bg-success-subtle border border-primary/20 p-6 text-center">
